...
PokerPlayHelp.prototype.getPokerWraper = function(pokerList){
	...
	for(var i = 0; i < pokerList.length; i++){
		...
		var pv = pm.getCardValue(poker);
		console.log("获取纸质 == "+pv)
		var count = pokerMap[pv];
		if(!count){
			count = 0;
		}
		pokerMap[pv] = ++count;
	...
	try{
		...
		return executor[executorKey](pokerMap, pokerWraper ,type);
	}catch (err){
		console.log(err);
		throw ExceptionType.WRONG_POKER_TYPE;
	}
...
/** 炸弹  */
Executor.prototype.bomb = function(map, pokerWraper,type){
	// console.log('aaa' + p);
	if(map.size() == 2){
		var index = 0;
...
/** 四带X  */
Executor.prototype.four = function(map, pokerWraper,type){
	for(var pokerValue in map){
		if(pokerValue == 'size')
			continue;
...
/**  连对  */
Executor.prototype.multiPair = function(map, pokerWraper,type){
...
/**  单张 */
Executor.prototype.one = function(map, pokerWraper,type){
...
/**  一对 */
Executor.prototype.pair = function(map, pokerWraper,type){
	var size = 0;
	var count = 0;
	var value = 0;
	for(pokerValue in map){
...
/**  飞机 */
Executor.prototype.plane = function(map, pokerWraper,type){
	var valueList = new Array();
	for(var pokerValue in map){
		if(pokerValue == 'size')
			continue;
...
/**  顺子 */
Executor.prototype.row = function(map, pokerWraper,type){
	var valueList = new Array();
	for(var pokerValue in map){
		if(pokerValue == 'size')
			continue;
...
/**  三带X */
Executor.prototype.three = function(map, pokerWraper,type){
	for(var pokerValue in map){
		if(pokerValue == 'size')
			continue;
